...
function Dashboard({
  ...
  const data = {
    ...
    datasets: [{
      label: "Wrong",
      data: instances.reverse().slice(instances.length - 5).map((instance) => instance.total - instance.score),
      backgroundColor: "rgba(255, 99, 132, 0.5)"
    }, {
      label: "Correct",
      data: instances.reverse().slice(instances.length - 5).map((instance) => instance.score),
